...
public class Tax {
	...
	public double getTax() {
		...
		for (i = 1; i < brackets[0].length; i++) {
			if (taxableIncome > brackets[filingStatus][i]) {
				tax = tax
						+ (brackets[filingStatus][i] - brackets[filingStatus][i - 1]) * rates[i];
			} 
			else {
				tax = tax
						+ (taxableIncome - brackets[filingStatus][i-1]) * rates[i];
				break;
	...
	public void printHeader() {
		...
		System.out.printf("%7s%52s%21s", "Taxable", "Married filing jointly", "Married filing");
		System.out.println();
		System.out.printf("%6s%20s%34s%16s%28s", "income", "Single filers", "or qualifying widow(er)", "separately", "Head of household");
		System.out.println();
